...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // --- Nodemailer (Zoho) ---
    // Pool kapalı: serverless'ta açık soket yüzünden fonksiyonun kapanmasını engelleyebilir.
    const transporter = nodemailer.createTransport({
      host: SMTP_HOST,                           // smtp.zoho.com
      port: Number(SMTP_PORT),                   // 587 (STARTTLS) ya da 465 (SSL)
      ...
      requireTLS: (SMTP_SECURE || "false") !== "true", // 587 kullanıyorsan STARTTLS zorunlu
      tls: {
    ...
    // Sadece sendMail, verify YOK. + timeout.
    await withTimeout(
    ...
    // serverless’ta açık soket bırakmamak için
